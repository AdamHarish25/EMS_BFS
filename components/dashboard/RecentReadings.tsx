          ...
          <tbody>
            {(Object.entries(groupedReadings) as [string, any[]][]).map(([date, items], groupIndex) => (
              <React.Fragment key={date}>
                ...
                <tr className="bg-slate-800/80 sticky top-[36px] z-[5] backdrop-blur-md">
                  <td colSpan={6} className="px-4 py-2 font-semibold text-slate-200 text-xs tracking-wider">
                    {date}
                  </td>
                </tr>
                {/* Data rows for this date */}
                {items.map((reading, i) => (
                  <tr key={`${groupIndex}-${i}`} className="border-b border-slate-800/50 hover:bg-slate-800/20 transition-colors">
